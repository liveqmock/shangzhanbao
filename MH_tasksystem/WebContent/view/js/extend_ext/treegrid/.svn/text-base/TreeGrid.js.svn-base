		...
		if (new_rc) {
			...
			for (i = 0, len = parents.length; i < len; i++) {
				...
				items.push(
					ts.brd_item.apply({
						id : rec.id,
						title : this.grid..breadcrumbs_tip,
						caption : rec.get(
			...
			this.breadcrumbs_el.update(
				this.grid..path_separator +
				ts.brd_item.apply({
					id: '',
					title : this.grid..breadcrumbs_root_tip,
					caption : this.grid.root_title
				}) +
				this.grid..path_separator +
				items.join(this.grid..path_separator)
			);
		}
		else {
			this.setRootBreadcrumbs();
...
Ext.ux.maximgb.treegrid.GridPanel = Ext.extend(Ext.grid.GridPanel, 
{
	...
	initComponent : function()
	{
		...
		Ext.applyIf(this., Ext.ux.maximgb.treegrid.GridPanel.prototype.);
		
		if (!this.root_title) {
			this.root_title = this.title || this..root_title;
		}
...
Ext.ux.maximgb.treegrid.GridPanel.prototype. = {
	path_separator : ' / ',
	root_title : '[root]',
