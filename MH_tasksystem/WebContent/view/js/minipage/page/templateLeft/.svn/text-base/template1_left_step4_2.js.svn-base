...
function gotoAnchor(info,url) {
	if($(info).val() == "请进一步描述产品的卖点，如果你感觉标语足够描述清楚了，可以不填详细描述"){
		$(info).val("");
	}
...
function getsubmit(){
	...
	if($("#slogan").val()=="请进一步描述产品的卖点，如果你感觉标语足够描述清楚了，可以不填详细描述"){
		$("#slogan").foucs();
		$("#slogan").val("")
	}
	//拼接json字符串
	var dataJson = "[{'componentid':'b664e9cc203e0a1642e05d60b055eb65','name':'产品标语的详细描述','content':'" + $("#slogan").val() 
	...
	ajaxUtil(url + "?dataJson=" + encodeURIComponent(dataJson), null,function msg(request, status, error) {
		window.location.href=root+"/view/pages/mini/page/templateLeft/template1_left_step4_3.jsp";
...
function imgupload(uploadImg,filePath){
	...
	$.ajaxFileUpload({
		...
		success: function (data, status){//服务器成功响应处理函数
			mainframe.getElementById("div1_image").src=root+filePath+"/"+data;
		}
	});
}
