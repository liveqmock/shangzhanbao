...
function getsubmit(){
	...
	//拼接json字符串
	var dataJson = "[{'componentid':'b664e9cc203e0a1642e05d60b055eb65','name':'产品卖点3的详细描述','content':'" + $("#reson3").val() 
	...
	ajaxUtil(url + "?dataJson=" + encodeURIComponent(dataJson), null,function msg(request, status, error) {
		window.location.href=root+"/view/pages/mini/page/templateLeft/template1_left_step4_6.jsp";
...
function imgupload(uploadImg,filePath){
	...
	$.ajaxFileUpload({
		...
		success: function (data, status){//服务器成功响应处理函数
			mainframe.getElementById("div4_image").src=root+filePath+"/"+data;
		}
	});
}
