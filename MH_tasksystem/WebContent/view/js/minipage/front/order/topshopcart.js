...
var userid = "";
//function onload(){
//	var checked = $("input[name='checkother']:checked");//判断复选框是否选中 (判断最后一个table中是否有数据，，没有数据的话，则整个table隐藏)
//	if(checked.length!=0){
//		$("#orderTable").show();
//	}
				...
				success : function(data) {
					...
					if(trnum == 1){
						this.obj.closest(".bingordersDiv").remove(); 
						 ...
						 var price1 = this.obj.closest("tr").find(".price").val();
...
function addToShopingCart(pid, num,pageid) {
	...
	for ( var i = 0; i < goods.length; i++) {
		...
		if (pid == goods[i].pid) {
			...
			if (goods[i].num > 99) { goods[i].num = 99; }falg=true; 
		}}
	}
	if(!falg){//不存在该产品,添加到购物车里面
				goods.push( { "pid" :"\"" + pid + "\"", "num" :"\"" + num + "\"","pageid" :"\""+pageid+"\"","userid" :"\""+userid+"\""});
	}
	goods = toJson(goods);
	$.cookie("shopcart", goods, {expires :10,path :'/'});
	alert('加入成功!');
 ...
 * 更改cookie中服务产品数量
 */
function changeProductNum(pid, num) {
	...
	for (; i < goods.length; i++) {// 说明存在
		if(userid==goods[i].userid){
			if (pid == goods[i].pid) {goods[i].num = num;break;}
		}
	}
	goods = toJson(goods);
	$.cookie("shopcart", goods, { expires :10, path :'/' });
...
function  saveTwoDomain(){
	...
	var json = getAsyncAjax(root+"/page/key/checkDomainIsUse?pageDatadomain="+encodeURIComponent(lastName), null);
	if(json==1){
		alert("该二级域名已存在！");
		return;
	}
	var value = "{'pageId':'"+divPageId+"','pageName':'"+pageName+"','lastName':'"+lastName+"'}";
...
function addNum(pid, i) {
	...
	changeProductNum(pid, newamount);
}
/**
 * 单机 - 时触发   （官方发布权限）
 * 
...
function addOtherNum(pid, i) {
	...
	$("#uuprice" + i).html("<input type='text' size='3' style='background: transparent; border: none; text-align: center;' value='"+unitPrice * newamount+".0"+"' name='orderData.price' title='uuprice"+i+"'>");//小计
	var countPrices = $("input[title^='uuprice']");
	var num = 0;
	for ( var i = 0; i < countPrices.length; i++) {
		num = parseInt(num) + parseInt(countPrices[i].value)
 ...
 * @return
 */
function changePageNum(pid, i){
	var unitPrice = $("#uunitPrice" + i).val();
    var amount = $("#uuamount" + i).val();
    if(amount==""){alert("数量不能为空");}
	if (amount.match(/\D/g,'')){alert('请输入正整数！')
		$("#uamount" + i).focus();
		return false;
		}
	...
	$("#uuprice" + i).html("<input type='text'  size='3' style='background: transparent; border: none; text-align: center;' value='"+unitPrice * amount+".0"+"' name='orderData.price' title='uuprice"+i+"'>");//小计
	var countPrices = $("input[title^='uuprice']");
	var num = 0;
	for ( var i = 0; i < countPrices.length; i++) {
		num = parseInt(num) + parseInt(countPrices[i].value)
				...
				$.ajax({
					...
					success : function(data) {
						...
						for ( var i = 0; i < ids.length; i++) {//批量清理cookie
							...
							$.cookie("shopcart", goods, {expires :10,path :'/'});//cookie 清除完毕后要提交
						}
						alert("操作成功,请查看[订单管理] 未付款订单!");
						$.cookie("shopcart", goods, {expires :10,path :'/'});
						$('<form method=post></form>').append($('<input type=hidden name=goods >').val(goods))
						.attr("action",root+"/order/key/orderList").appendTo($(document.body)).submit();	
					}
				});
			}
		}
	...
	if(checked.length==0){
		...
		if(checkZero("uuamount")){//验证购物车订单为空
			...
			if(confirm("确认结算?")){
				...
				uproductconfigname.push($("#uproductconfigname"+i).val());
				...
				uprice=uprice.join(",")
				var value = "ids="+ids+"&productName="+uproductname+"&productConfigName="+
				uproductconfigname+"&unitPrice="+unitprice+"&amount="+uamount+"&price="+uprice
				var url = root + '/shop_cart/key/otherOrderSubmit?'+encodeURIComponent(value);
...
function shoppingCartDivPage(pageId){
