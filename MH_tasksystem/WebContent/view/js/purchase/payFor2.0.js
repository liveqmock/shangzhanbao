$(function(){
    ...
    openId1=GetQueryString("openId");  //截取openId参数
    addCookie("openId",openId1,600000 ,"/"); //10分钟时效  
	...
	// 初始化加载，如果购买组件存在，就把次div添加到页面
	 if($("#goumaiwraper").html()!=""){
		...
		/*
		 * "<div class=\"order_div1\"><span>商品品类:</span><label
		 * id=\"order_goodstype\"></label></div>" +
		 */
		...
		/*
		 * "<div class=\"order_div1\"><span>运费信息:</span><label
		 * id=\"order_goodsyunfei\" style=\"color: #FF0000\"></label></div>" +
		 */
			...
			if(paytype=="0"){
			 ...
			 // 是微信浏览器
			  if(openId=="null" || openId==""){
				// 微信支付
				if(paytype=="1"){
					var weixinurl=$("#objectRoot").val()+"/order_manager/key/toweixinPayOrder?" +
					"conSumerUserData.weixinOpenId="+openId+"&price="+money+"&paytype="+paytype;
					$.ajax({
						type:"POST",
						url:weixinurl,
						data : $("#toPayOrder").serialize(),
						dataType : 'text',// 返回值类型 一般设置为json
						success : function(orderdata) {
							if(orderdata!="1"){
								 var json = eval("(" + orderdata + ")");
								 var money=json.price;// 订单价钱
								 var orderNo=json.orderCode; // 订单编号
								 // 当订单价格为0时直接跳转到订单支付成功页面
								 if(money=="0"){
									 var redPackageId=$("#redPackageId").val();
									 var cId=$("#cId").val();
									 var sId=$("#sId").val();
									 updateconSumerOrderData(orderNo,redPackageId,cId,sId);  // 去订单支付成功页面
								 }else{
									 weixinconSumerOrderData(orderdata); // 跳微信生成订单
								 }
								}
							}
						})
		    	...
		    	if(result.redPackage!="1"){
			        ...
			        for (var i = 0; i < list.length; i++) {
						option +="<option value=\""+list[i].snarchPrice.trim()+"\" " +
								...
								+list[i].redPackageName.trim()+list[i].snarchPrice.trim()+"元</option>"
					}
...
function selectRed(s)
{
    ...
    if($("#changDis").val()!="false"){
    	...
    	if(onePrice > redPrice){
    		...
    		$("#order_goodsprice").html(changeTwoDecimal(sumprice)));
    	}else{
 ...
 * @param con
 */
function zhifuonClick(con){
		var id=$(con).attr("id");// 当前选择id
		$(".zhifu").find(".redsolid").each(function(i){
		  if($(this).attr("id")==id){
			    // 当前选中或者不选择样式切换
				  if($(this).find("div").attr("class")=="redsolid_div"){
					  $(this).find("div").removeAttr("class","redsolid_div");   // 删除
																				// 选择样式
				  }else{
					 $(this).find("div").attr("class","redsolid_div");   // 添加
																			// 选择样式
				  }
...
function selectaccountNumber(){
	...
	if(userId!=null){
		$.ajax({
			type:"POST",
			url:$("#objectRoot").val() +"/order_manager/key/selectaccountNumber?userId="+userId,
			dataType : 'text',// 返回值类型 一般设置为json
			success : function(data) {
			  if(data!=1){
				  var zhifuHtml="";
				  var list = eval("(" + data + ")");
					for(var i in list){ 
						if(list[i].accountType==0){
							zhifuHtml +="<div class='div4'><img src='../../../images/mini/purchase/goumai/u138.png' />" +
							"<span class=\"redsolid\" id='"+i+"' onclick='zhifuonClick(this)' data='"+list[i].accountType+"'>" +
							"<div data='"+list[i].accountType+"' ></div></span></div></div>";
						}
						if(list[i].accountType==1 && isWeixin()){
							zhifuHtml +="<div class='div4'><img src='../../../images/mini/purchase/goumai/weiixnzhifu.png' />" +
							"<span class=\"redsolid\" id='"+i+"' onclick='zhifuonClick(this)' data='"+list[i].accountType+"'>" +
							"<div data='"+list[i].accountType+"' class='redsolid_div'></div></span></div></div>";
						}
					}
				  }else{
					  $(".zhifu").find(".div9").before("<div class=\"error\">该商户还没未添加支付方式，请联系商家！</div>");
				  }	
			  $(".zhifu").find(".div9").before(zhifuHtml);
 ...
 * 跳微信生成订单
 */
function weixinconSumerOrderData(orderdata){
	 var json = eval("(" + orderdata + ")");
	 var describe=json.goodsName;  // 商品名称
	 var openId=json.weixinOpenId;  // openid
	 var userId=json.conSumerUserId;// 消费者id
	 var orderNo=json.orderCode; // 订单编号
	 var money=json.price;// 订单价钱
	 var redpackageid=json.conSumerUserData.redPackageId; // 红包id
	 var cid=json.conSumerUserData.cId;// 领红包id
	 var sid=json.conSumerUserData.sId;// 抢红包id
	 var redpackageStr="";
	  if(redpackageid!=""){
		  redpackageStr = redpackageid+","+cid+","+sid;
	  }
	  if(redpackageStr==""){
		  redpackageStr="1";
	  }
	 var url=$("#objectRoot").val() +"/to_wx_pay/key/wxPay?describe="+describe+"&openId="+openId+"&userId="+redpackageStr+"&orderNo="+orderNo+"&money="+money;
	$.ajax({
		type:"POST",
		url:url,
		data : $("#toPayOrder").serialize(),
 ...
 * 微信支付接口
 */
function getWeiXinPrepayId(data){
	var json = eval("(" + data + ")");
	var appid=json.appid;  // 商户工众平台id
	var timeStamp=json.timeStamp;  // 系统时间
	var nonceStr=json.nonceStr; // 随机字符串
	var package_=json.package_; // 微信预支付id
	var sign=json.sign;  // 微信签名
	alert(package_);
	 WeixinJSBridge.invoke('getBrandWCPayRequest',{
  		 "appId" : appid,"timeStamp" : timeStamp, "nonceStr" : nonceStr, "package" : package_,"signType" : "MD5", "paySign" : sign 
   			},function(res){
 ...
 * 订单支付成功
 */
function updateconSumerOrderData(){
	var url = root + "/order_manager/key/paySuccessWithOrderChangeState?conSumerOrderData.orderCode="+document.getElementById("out_trade_no").value;
	$.ajax({
		type : 'POST',
		url : url,
		dataType : "text",
 ...
 * 订单支付成功跳转页面
 */
function updateconSumerOrderData(orderNo,redPackageId,cId,sId){
	var url = $("#objectRoot").val() + "/order_manager/key/topayForSuccess_order?conSumerOrderData.orderCode="+orderNo+
	"&redPackageId="+redPackageId+"&cId="+cId+"&sId="+sId;
