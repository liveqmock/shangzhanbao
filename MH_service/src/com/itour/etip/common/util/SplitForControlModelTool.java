 ...
 * 
 */
public class SplitForControlModelTool {
	...
	public void split(SplitableData source, String extendWhereConditions) {
		...
		+ " where productControl.effectBeginTime <= to_date('" + endDateStr
		+ "','yyyyMMdd') and productControl.effectEndTime>=to_date('" + startDateStr
		+ "','yyyyMMdd')" + extendWhereConditions;
	...
	public void saveSplitableDataByJson(JSONObject json, Class splitableClass, String resourceType) {
		...
		for (String id : items) {
			...
			extendWhereConditions.append(" and productControl.approvalState = ").append(
					data.getApprovalState());
			if(data.getReleaseState() == 3 || data.getReleaseState() == 4){
				extendWhereConditions.append(" and productControl.releaseState = 2");//只有发布状态的可以失效
			}else{
				extendWhereConditions.append(" and productControl.releaseState = ").append(
	...
	private SplitForControlModelTool() {
		splitableDataPersistence = (ISplitablePersistence)SpringContextHelper.getBean("splitableDataPersistence");
	}

	private static SplitForControlModelTool instance = null;

	public static SplitForControlModelTool getInstance() {
		if (instance == null) {
			instance = new SplitForControlModelTool();
		}
		return instance;
	}
